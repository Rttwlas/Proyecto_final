...
def BusquedaLaboral(request):
      if request.method == 'POST':
            ...
            if miFormulario.is_valid:
                  ...
                  busqueda = Busqueda(nombre=informacion['nombre'], apellido=informacion['apellido'] , comentario=informacion['comentario'] , mail=informacion['mail'])              
                  busqueda.save()
...
class BazarEliminar(DeleteView):
      model = Bazar
      template_name = "AppBazar/bazar.html"
      fields = ['nombre','foto','precio']
      success_url = reverse_lazy('formulario')
